...
import { canUserAccessApp, getUserStatus, USER_APPROVAL_STATUSES } from "@/lib/userApproval";
import { getAppBaseUrl, notifyAdminsNewUserApproval } from "@/lib/emailNotifications";

class AccountPendingApproval extends CredentialsSignin {
...
async function upsertOAuthUser({ email, name, image }) {
  ...
  }).catch((err) => console.error("Admin OAuth user notification error:", err));
    ...
    Credentials({
      ...
      async authorize(credentials, request) {
        ...
        if (role !== "admin" && role !== "manager") {
          if (status === USER_APPROVAL_STATUSES.PENDING) throw new AccountPendingApproval();
          if (status === USER_APPROVAL_STATUSES.REJECTED) throw new AccountRejected();
          if (status === USER_APPROVAL_STATUSES.INACTIVE) throw new AccountBlocked();
        }
  ...
  callbacks: {
    async signIn({ user, account }) {
      if (account?.provider === "google") {
        ...
        const { db } = await getDB();
        const fullUser = await db.collection("users").findOne({ userId: dbUser.uid });
        if (!canUserAccessApp(fullUser)) return false;
      }
      return true;
    },
