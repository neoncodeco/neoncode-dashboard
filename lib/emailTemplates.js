  ...
  const ctaBlock =
    ctaLabel && ctaUrl
      ? `<table role="presentation" cellspacing="0" cellpadding="0" style="margin:22px 0 8px;">
        ...
        <p style="margin:0 0 6px;color:#94a3b8;font-size:12px;line-height:1.5;">Or copy this link:</p>
        <p style="margin:0 0 18px;color:#64748b;font-size:12px;word-break:break-all;">${escapeHtml(ctaUrl)}</p>`
      : "";
...
export function buildNeonEmailText({ title, greeting, paragraphs = [], details = [], statusText, ctaUrl, footerNote }) {
  ...
  if (ctaUrl) lines.push("", `Open: ${ctaUrl}`);
  lines.push("", footerNote || process.env.NOTIFICATION_EMAIL_FOOTER || "Automated message from Neon Code.");
  return lines.join("\n");
}
