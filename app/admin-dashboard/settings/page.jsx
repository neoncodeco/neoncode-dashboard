...
export default function TokenSettings() {
  ...
  const getClientInfo = (metaId) => {
    ...
    return expandAdAccountRequests(clientRequests).find(req => req.MetaAccountID?.trim() === metaId.trim());
  };
          ...
          <div className="space-y-6">
            {bmConfigs.map((bm, bmIndex) => (
              <div key={bmIndex} className="bg-[#0d1d3b] rounded-[2.5rem] border border-[#1e2d4d] shadow-2xl overflow-hidden transition-all">
                  ...
                  <div className="flex items-center gap-5">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">
                        Business ID: {bm.businessId || "Unset"} • {bm.slots?.length || 0} Slots Attached
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-4">
                    <button onClick={(e) => { e.stopPropagation(); removeBMRow(bmIndex); }} className="text-red-400/50 hover:text-red-500 transition-colors">
                      <Trash2 size={18} />
                    </button>
                ...
                {/* BM Content - সব কিছু এই একটা div এর ভেতরে */}
                {expandedBM === bmIndex && (
                  <div className="p-8 md:p-10 space-y-8 border-t border-[#1e2d4d]">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <input
                        placeholder="BM Name"
                        value={bm.bmName}
                        onChange={(e) => handleBMChange(bmIndex, "bmName", e.target.value)}
                        className="w-full p-4 bg-[#081227] border border-[#1e2d4d] rounded-2xl text-sm font-semibold text-white focus:border-blue-500 outline-none"
                      />
                      <input
                        placeholder="Business ID"
                        value={bm.businessId}
                        onChange={(e) => handleBMChange(bmIndex, "businessId", e.target.value)}
                        className="w-full p-4 bg-[#081227] border border-[#1e2d4d] rounded-2xl text-sm font-semibold text-white focus:border-blue-500 outline-none"
                      />
                    </div>
                    <textarea
                      value={bm.token}
                      onChange={(e) => handleBMChange(bmIndex, "token", e.target.value)}
                      rows={2}
                      placeholder="EAA..."
                      className="w-full p-5 bg-[#081227] border border-[#1e2d4d] rounded-2xl text-xs font-mono text-blue-300 resize-none shadow-inner"
                    />
                    ...
                    <div className="space-y-4">
                      <div className="flex justify-between items-center">
                        <h3 className="text-xs font-black text-slate-500 uppercase tracking-widest flex items-center gap-2">
                          <LayoutGrid size={14} /> Attached Ad Slots
                        </h3>
                        <button
                          onClick={() => addSlot(bmIndex)}
                          className="text-[10px] font-black bg-blue-600/10 text-blue-400 border border-blue-500/20 px-4 py-2 rounded-xl hover:bg-blue-600 hover:text-white transition-all"
                        >
                          + Add New Slot
                        </button>
                      </div>
                        ...
                        <table className="w-full text-left bg-[#081227]/40">
                          <thead className="bg-[#132546]/50 text-[10px] font-black text-slate-500 uppercase">
                            <tr>
                              ...
                              <th className="px-6 py-4">Connected Profiles</th>
                    ...
                    <div className="flex justify-end pt-2 border-t border-[#1e2d4d]">
                      <button
                        onClick={() => handleUpdateBM(bmIndex)}
                        disabled={loading === bmIndex}
                        className="bg-[#8ab4ff] text-[#081227] flex items-center gap-3 px-8 py-4 rounded-[1.5rem] font-black shadow-xl hover:-translate-y-1 active:scale-95 transition-all disabled:opacity-50 text-sm"
                      >
                        {loading === bmIndex ? (
                          <><Loader2 className="animate-spin" size={16} /> Saving...</>
                        ) : (
                          <><Save size={16} /> Save {bm.bmName || `BM ${bmIndex + 1}`}</>
                        )}
                      </button>
                    </div>
          ...
          <div className="bg-[#0d1d3b] p-8 rounded-[2.5rem] border border-[#1e2d4d] shadow-xl space-y-6">
            ...
            <div className="space-y-4 text-xs font-bold uppercase tracking-tighter">
              <div className="flex justify-between py-2 border-b border-[#1e2d4d]">
                ...
                <span className="text-emerald-500 flex items-center gap-2">Healthy</span>
              </div>
              <div className="flex justify-between py-2 border-b border-[#1e2d4d]">
                <span className="text-slate-500">Active BMs</span>
                <span className="text-white">{bmConfigs.length} Nodes</span>
              </div>
              <div className="flex justify-between py-2">
                <span className="text-slate-500">Total Slots</span>
                <span className="text-blue-400">{bmConfigs.reduce((acc, curr) => acc + (curr.slots?.length || 0), 0)} Linked</span>
              </div>
            </div>
          </div>
