...
//   const [bmConfigs, setBmConfigs] = useState([{ bmName: "", businessId: "", token: "" }]);
...
//         if (data.bmConfigs && data.bmConfigs.length > 0) setBmConfigs(data.bmConfigs);
//         else if (data.token) setBmConfigs([{ bmName: "Primary BM", businessId: "", token: data.token }]);
//         setUsdToBdtRate(String(data.usdToBdtRate || 150));
//       }).finally(() => setInitialLoading(false));
//   }, [token]);

//   const addBMRow = () => setBmConfigs([...bmConfigs, { bmName: "", businessId: "", token: "" }]);
...
//       });
//       if (res.ok) Swal.fire({ icon: "success", title: "Updated", text: "Configuration saved.", timer: 1500, showConfirmButton: false });
//     } catch (err) { Swal.fire({ icon: "error", title: "Error", text: "Update failed." }); }
//     finally { setLoading(false); }
//   };

//   if (initialLoading) return <div className="flex items-center justify-center min-h-screen text-white"><Loader2 className="animate-spin mr-2" /> Initializing...</div>;
...
//           <div>
//             <h1 className="text-2xl md:text-4xl font-black text-white tracking-tight">System Controls & API</h1>
//             <p className="text-slate-400 mt-2 font-medium">Manage backend configurations and security keys.</p>
//           </div>
//           <button onClick={addBMRow} className="flex items-center gap-2 bg-[#1e2d4d] hover:bg-[#2c4167] text-white px-6 py-3 rounded-2xl font-bold transition-all border border-[#2c4167]">
...
//             {/* --- MULTI BM LIST --- */}
//             <div className="bg-[#0d1d3b] rounded-[2.5rem] border border-[#1e2d4d] shadow-2xl overflow-hidden p-8 md:p-12 space-y-10">
              
//               {/* Global Rate */}
//               <div className="relative group">
//                 <div className="flex justify-between items-center mb-3">
//                   <label className="text-[11px] font-black text-slate-500 uppercase tracking-[0.2em]">Global USD to BDT Rate</label>
//                   <span className="bg-[#1a2e56] text-[#8ab4ff] rounded-md px-2 py-0.5 text-[10px] font-bold">1 USD = Tk {usdToBdtRate}</span>
//                 </div>
//                 <input type="number" value={usdToBdtRate} onChange={(e) => setUsdToBdtRate(e.target.value)} className="w-full p-6 bg-[#081227] border border-[#1e2d4d] rounded-[2rem] text-xl font-bold focus:outline-none focus:border-[#8ab4ff] transition-all text-white shadow-inner" />
//               </div>

//               {bmConfigs.map((bm, index) => (
//                 <div key={index} className="space-y-6 pt-10 border-t border-[#1e2d4d] relative">
//                   {index > 0 && <button onClick={() => removeBMRow(index)} className="absolute top-10 right-0 text-red-400 hover:text-red-500"><Trash2 size={20}/></button>}
                  
//                   <div className="flex items-center gap-4">
//                     <div className="bg-blue-600 rounded-2xl p-4 shadow-xl shadow-blue-900/20"><Terminal size={28} /></div>
//                     <div>
//                       <h2 className="text-xl font-black text-white">Facebook BM Token #{index + 1}</h2>
//                       <p className="text-sm text-slate-500 font-bold uppercase tracking-tighter">Integration Key Management</p>
//                     </div>
//                   </div>

//                   <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
//                     <input placeholder="BM Display Name" value={bm.bmName} onChange={(e) => handleInputChange(index, "bmName", e.target.value)} className="w-full p-5 bg-[#081227] border border-[#1e2d4d] rounded-2xl text-sm font-semibold focus:outline-none focus:border-[#8ab4ff]" />
//                     <input placeholder="Business ID" value={bm.businessId} onChange={(e) => handleInputChange(index, "businessId", e.target.value)} className="w-full p-5 bg-[#081227] border border-[#1e2d4d] rounded-2xl text-sm font-semibold focus:outline-none focus:border-[#8ab4ff]" />
//                   </div>

//                   <div className="relative group">
//                     <textarea value={bm.token} onChange={(e) => handleInputChange(index, "token", e.target.value)} rows={6} placeholder="Paste your system token starting with EAA..." className="w-full p-8 bg-[#081227] border border-[#1e2d4d] rounded-[2.5rem] text-sm font-mono focus:outline-none focus:border-[#8ab4ff] transition-all resize-none text-slate-300 shadow-inner leading-relaxed" />
//                     <ShieldCheck className="absolute bottom-8 right-8 text-emerald-500" size={28} />
//                   </div>
//                 </div>
//               ))}

//               {/* SECURITY PROTOCOL SECTION (From Image) */}
//               <div className="bg-[#102143]/50 flex items-start gap-4 rounded-[2rem] border border-[#1e2d4d] border-dashed p-6">
//                 <div className="rounded-xl bg-[#203963] p-2"><AlertCircle size={20} className="text-[#dce8ff]" /></div>
//                 <div>
//                   <p className="text-sm text-white font-black">Security Protocol</p>
//                   <p className="text-xs text-slate-400 leading-relaxed mt-1 font-medium italic">
//                     By updating these tokens, you acknowledge that all current Facebook Graph API requests will use these new keys immediately. Make sure tokens are from a verified System User with <b>Full Control</b>.
//                   </p>
//                 </div>
//               </div>

//               <div className="pt-8 border-t border-[#1e2d4d] flex justify-end">
//                 <button onClick={handleUpdate} disabled={loading} className="bg-[#8ab4ff] hover:bg-[#a3c5ff] text-[#081227] flex w-full items-center justify-center gap-4 rounded-[2rem] px-12 py-5 text-sm font-black transition-all hover:-translate-y-[2px] active:scale-95 disabled:opacity-50 md:w-auto shadow-lg shadow-blue-500/10">
...
//                   <p className="text-slate-400 text-xs leading-relaxed font-medium">Need help with permissions? Visit Meta Developer documentation to learn more about ads_management scopes.</p>
//                   <button className="bg-[#8ab4ff] text-[#081227] rounded-xl px-6 py-3 text-xs font-black uppercase tracking-widest transition-all hover:scale-105">VIEW DOCUMENTATION</button>
//                 </div>
//               </div>
...
//                   <h3 className="text-lg font-black text-white">Privacy Focus</h3>
//                 </div>
//                 <p className="text-slate-400 text-xs leading-relaxed font-medium">All tokens are stored using high-level AES encryption on the database. No one can see your full token once saved.</p>
//               </div>
//             </div>
//           </div>
...
//             <div className="bg-[#0d1d3b] p-8 rounded-[2.5rem] border border-[#1e2d4d] shadow-sm space-y-6">
//                <h3 className="text-xs font-black uppercase tracking-[0.2em] text-slate-500 flex items-center gap-2"><Server size={14} /> Global Status</h3>
//                <div className="space-y-4 text-xs">
//                   <div className="flex justify-between py-2 border-b border-[#1e2d4d]"><span className="text-slate-500 font-bold uppercase">API Status</span><span className="text-emerald-500 font-black flex items-center gap-2"><div className="w-1.5 h-1.5 bg-emerald-500 rounded-full"></div> HEALTHY</span></div>
//                   <div className="flex justify-between py-2 border-b border-[#1e2d4d]"><span className="text-slate-500 font-bold uppercase">Environment</span><span className="bg-[#1a2e56] text-[#8ab4ff] px-3 py-1 rounded-lg font-black uppercase text-[10px]">Production</span></div>
//                   <div className="flex justify-between py-2"><span className="text-slate-500 font-bold uppercase">Node Version</span><span className="text-white font-black">v20.x</span></div>
//                </div>
//             </div>

//             <div className="bg-[#0d1d3b] flex flex-col items-center rounded-[2.5rem] border border-[#1e2d4d] p-8 text-center">
...
//                 <p className="px-4 text-[10px] font-bold uppercase leading-relaxed text-[#9fb3de]">System tracking is enabled for all configuration changes.</p>
//             </div>
//           </div>
//         </div>
//       </div>
